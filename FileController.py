...
def upload(filename):
    ...
    #print("原filename",filename)
    #print("filename",cipher_filename)
...
def download(filename):
    ...
    username = config.GLOBAL_CONFIG['username']
    username_hashed = hashlib.sha256(f"{username}".encode("utf-8")).hexdigest()
    #print("userhash",username_hashed)
...
def delete(filename):
    ...
    response_raw = requests.delete(base_url + suffix, params=data_send, headers=headers)
    response = response_raw.json()
    if response["status"] == "success":
...
def edit(filename,updated_content):
    ...
    response_raw = requests.put(base_url + suffix, json=data_send, headers=headers)
    response = response_raw.json()
    if response["status"] == "success":
...
def share(filename, to_user):
    ...
    response = response_raw.json()
    cipher_content=bytes.fromhex(response.get('encrypted_content', response.get('content', '')))
    content=decrypt_with_private_key(cipher_content,private_key)
    ano_public_key_str=response.get('target_public_key', response.get('public_key', ''))
    ano_public_key_pem=base64.b64decode(ano_public_key_str)
    ano_public_key=serialization.load_pem_public_key(ano_public_key_pem)
    ano_cipher_content=encrypt_with_public_key(content, ano_public_key).hex()
    confirm_share(filename,to_user,ano_cipher_content)

def confirm_share(filename,to_user,cipher_content):
    ...
    #content=bytes.fromhex(decrypt_with_private_key(bytes.fromhex(cipher_content),private_key))
    #ano_cipher_content=encrypt_with_public_key(content,ano_public_key)

    data_send_2={
        "action":"confirm_share",
        "encrypted_content": cipher_content,
        "filename":cipher_filename,
        "username": username_hashed,
        "to_user": to_user_hashed,
        "sign":signature,
